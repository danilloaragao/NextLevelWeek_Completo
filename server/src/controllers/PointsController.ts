...
class PointsController {
    ...
    async show(request: Request, response: Response) {
        ...
        if (!point)
            return response.status(404).json({ status: 404, message: 'Ponto de coleta não encontrado' })

        const point_items = await knex('items').select('title', 'image')
            .join('point_items', 'items.id', "=", "point_items.item_id")
            .where('point_items.point_id', id)

        const serializedPoint = {
            ...point,
            image: `http://192.168.15.14:3333/uploads/${point.image}`,
            point_items
        }
